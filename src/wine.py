...
def readfilter(filt):
  ...
  m2cm  = 1e4 # Microns to cm conversion factor
...
def readkurucz(kfile, temperature, logg):
  ...
  inten, freq, grav, temp, nainten, head = kurucz_inten.read(kurucz_file,
                                                             freq=True)  
  # Wavenumber in cm^-1
  ...
  tmodel = temp[np.argmin(np.abs(kout[3]-temperature))]
  # Nearest sampled log(g):
  gmodel = grav[np.argmin(np.abs(grav-gsun))]
...
def resample(specwn, filterwn, filtertr, starwn, starfl):
  ...
  # wavenumber range:
  wnindices = np.where((specwn < filrerwn[-1]) & (filterwn[0] < specwn))

  # Make function to spline-interpolate the filter and stellar flux:
  finterp = si.interp1d(bandwn, bandtr)
  ...
  nifilter = ifilter/np.trapz(ynew, specwn[wnindices])
...
def bandintegrate(spectrum, specwn, nifilter, wnindices):
  ...
  return np.trapz(spectrum[wnindices]*nifilter, specwn[wnindices])
